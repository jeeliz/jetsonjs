...
const JETSONJSCLIENT = (function(){
  ...
  let _callbackConnect = null, _isClientConnected = -1; // -1,0 or 1
  ...
  function update_wifiConfig(event){
    ...
    const update_val = function(valKey, domKey){
      const domElt = _wifiConfig.domElements[domKey];
      if (!domElt) return;
  ...
  const that = {
    'init': function(spec){ // entry point
      ...
      if (spec.wifiConfigIds){
        ...
        Object.keys(_wifiConfig.domElements).forEach(function(domKey){
          const domId = spec.wifiConfigIds[domKey];
    ...
    'close': function(){
      ...
      try {
        ...
        const electronWin = electronRemote.getCurrentWindow();
        electronWin.close();
      } catch(e){
